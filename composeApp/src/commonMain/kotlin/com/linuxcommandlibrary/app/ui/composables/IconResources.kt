...
fun BasicGroup.getIconId(): AppIcon = when (description) {
    ...
    // Shell Scripting
    "Script Basics" -> AppIcon.INFO_SMALL

    "Variables" -> AppIcon.SHOW_PROPERTY

    "Special Variables" -> AppIcon.FLASH

    "String Operations" -> AppIcon.TEXT

    "Conditionals" -> AppIcon.FLOW_CHART

    "Loops" -> AppIcon.REFRESH

    "Functions" -> AppIcon.PUZZLE

    "Arrays" -> AppIcon.NUMBERED_LIST

    "Arithmetic" -> AppIcon.KITCHEN_SCALES

    "Input and Output" -> AppIcon.KEYBOARD

    "Exit Codes and Traps" -> AppIcon.CANCEL

    // Tmux
    "Getting Started" -> AppIcon.PLAY

    "Sessions" -> AppIcon.DESKTOP

    "Windows" -> AppIcon.RESTORE_WINDOW

    "Panes" -> AppIcon.COLLAPSE_ALL

    "Resizing Panes" -> AppIcon.ARROW_UPWARD

    "Copy Mode" -> AppIcon.CLIPBOARD

    "Command Mode" -> AppIcon.CONSOLE

    // Regular Expressions
    "Basic Matching" -> AppIcon.LOUPE

    "Anchors" -> AppIcon.FILE_LINK

    "Character Classes" -> AppIcon.LIST_GROUPS

    "POSIX Classes" -> AppIcon.LINUX

    "Quantifiers" -> AppIcon.PLUS

    "Groups and Alternation" -> AppIcon.GROUP_SELECTED

    "Escaping" -> AppIcon.UNDO

    "Basic vs Extended Regex" -> AppIcon.DOCUMENT

    "Common Examples" -> AppIcon.IDEA
